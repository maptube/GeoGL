...
//const int Shader::v_inPosition = 0;
//const int Shader::v_inColor = 1;
...
namespace gengine {
	...
	*/
	...
	///
	/// </summary>
	/// <param name="vsFile">Filename of vertex shader code</param>
	/// <param name="fsFile">Filename of fragment shader code</param>
	/// <param name="vao">Vertex Array Object containing the names and indices of the vertex arrays that relate to the shaders</param>
	Shader::Shader(const char *vsFile, const char *fsFile) {
		...
		_shaderUniforms = new ShaderUniformCollection();
		_shaderAttributes = new ShaderAttributeCollection();
	...
	*/
	void Shader::init(const char *vsFile, const char *fsFile) {
		...
		//http://ogltotd.blogspot.co.uk/2006/12/checking-for-d3d-shader-version.html
		...
		//get the following from the vao object
		...
		//automatically query the shaders for the uniforms which need to be bound
		_shaderUniforms->Clear();
		_shaderUniforms->BuildFromShaderProgram(shader_id);
		_shaderUniforms->Debug_Enumerate(); //write out what we found to the console

		//automatically query the shaders for the attributes which need to be bound
		_shaderAttributes->Clear();
		_shaderAttributes->BuildFromShaderProgram(shader_id);
		_shaderAttributes->Debug_Enumerate(); //write out what we found to the console
	}
	...
	*/
	Shader::~Shader() {
		if (!OGLDevice::hasProgrammableShaders) return;
		...
		//clean up attributes and uniforms
		delete _shaderUniforms;
		delete _shaderAttributes;
	}
