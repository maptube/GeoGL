...
const double FlyController::rateX = 0.0001; //rates applied to cursor position for aileron
const double FlyController::rateY = 0.0001; //rates applied to cursor position for elevator
...
void FlyController::Forward()
{
	...
	mCam = glm::rotate(mCam,-Aileron,glm::dvec3(0,0,1));
	mCam = glm::rotate(mCam,Elevator,glm::dvec3(1,0,0));

	//move forwards along view
	mCam = glm::translate(mCam,glm::dvec3(0,0,-Speed)); //OpenGL Z -ve is into screen
...
void FlyController::CursorPosCallback(GLFWwindow *window, double mx, double my)
{
	...
	P.x/=(float)width;
	P.y/=(float)height;
	P=P*2.0-1.0; //set range to +-1 in both axes
